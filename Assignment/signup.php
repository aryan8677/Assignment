...
if($emailcount>0)
{
    ...
    if($password === $confirmpassword){
        $insertquery = "insert into users(fname,lname,email,password,verification_code,is_verified,contactno) values('$fname','$lname','$email','$password','$confirmpassword','$verification_code','notverified',$contact')";   
        $iquery = mysqli_query($con, $insertquery);

        if($iquery){

            $to_email = "codearyan1@gmail.com";
            $subject = "Verification Code";
            
            $body = "Hi. $fname.  click here
            http://localhost/loginsystem/email_verification.php? verification_code = $verification_code";
            
            $header = "From:abhardwaz79@gmail.com ";
       
            
            $retval = mail ($to,$subject,$body,$header);
            
            if( $retval == true ) {
                $_SESSION['msg'] = "check your mail for veify your account $email";
